...
router.patch('/:username', authUser, requireLogin, async function(
  ...
  try {
    ...
    let fields = { ...req.body };
    delete fields._token; // Assuming `_token` should be ignored if present in the body
...
router.delete('/:username', authUser, requireAdmin, async function(
  ...
  try {
    await User.delete(req.params.username); // Ensure to await the delete operation
