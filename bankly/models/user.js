...
class User {
   ...
   * Returns all user data if true, throws error if invalid
   */
   ...
   */
  static async getAll() {
    //======Fixed Bug1: Removed unnecessary parameters (username, password)======
    const result = await db.query(
      `SELECT username,
                first_name,
                last_name,
  ...
  static async get(username) {
    ...
    if (!user) {
      //======Fixed Bug2: Added 'throw' to ensure the error is properly raised=========
